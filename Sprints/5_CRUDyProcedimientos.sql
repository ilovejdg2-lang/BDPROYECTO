...
BEGIN
    ...
    VALUES (COALESCE(@fecha_matricula, GETDATE()), @estado_matricula, @id_estudiante);
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO Matricula (fecha_matricula, estado_matricula, id_estudiante)
        VALUES (GETDATE(), 'Activa', @id_estudiante);
        ...
        WHERE TRY_CAST(value AS INT) IS NOT NULL
          AND EXISTS (SELECT 1 FROM Asignatura a WHERE a.codigo_interno_asignatura = TRY_CAST(value AS INT));
   ...
   BEGIN CATCH
    ...
    RAISERROR('Error al matricular: %s', 16, 1, @mensaje_error);
END CATCH
END
GO
